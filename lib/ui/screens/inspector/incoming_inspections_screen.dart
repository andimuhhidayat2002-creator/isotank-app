...
class IncomingInspectionsScreen extends StatefulWidget {
  const IncomingInspectionsScreen({super.key});

  @override
  State<IncomingInspectionsScreen> createState() => _IncomingInspectionsScreenState();
}

class _IncomingInspectionsScreenState extends State<IncomingInspectionsScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Incoming Inspections'),
      ...
      body: FutureBuilder<List<dynamic>>(
        ...
        builder: (context, snapshot) {
          ...
          // Filter ONLY incoming inspections
          final incomingJobs = allJobs.where((job) {
            final activityType = job['activity_type']?.toString().toLowerCase() ?? '';
            return activityType.contains('incoming') || activityType == 'inspection_incoming';
          }).toList();
          ...
          return _InspectionList(jobs: incomingJobs, onRefresh: _loadJobs);
        },
      ),
    );
  }
                              ...
                              Padding(
                                ...
                                child: Row(
                                  children: [
                                    ...
                                    const SizedBox(width: 4),
                                    Text('From: ${job['origin']}', style: const TextStyle(color: Colors.white70)),
                                  ],
                                ),
                              ),
                          ],
